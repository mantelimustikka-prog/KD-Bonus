...
class KD_Bonus_Rewards {
	...
	public function register() {
		...
		add_action( 'admin_post_kd_bonus_copy_status_bfw', array( $this, 'handle_copy_status_from_bfw' ) );
		...
		<table class="form-table" role="presentation" style="background:#fff8c5;border:1px solid #dcdcde;border-radius:8px;padding:12px 16px;">
			<tbody>
				...
				<?php if ( $can_manage ) : ?>
					...
					<tr>
						<th><?php esc_html_e( 'Copy from Bonus For Woo', 'kd-bonus' ); ?></th>
						<td>
							<?php $this->render_copy_status_from_bfw_button( $user->ID ); ?>
						</td>
					</tr>
				<?php endif; ?>
				<tr>
	...
	/**
	 * Render the "Copy Status from Bonus For Woo" button and a redirect-back form.
	 *
	 * @param int $user_id Target user ID.
	 */
	public function render_copy_status_from_bfw_button( $user_id ) {
		$user_id = absint( $user_id );
		if ( $user_id <= 0 || ! $this->can_manage_user_bonus( $user_id ) ) {
			return;
		}

		$is_bfw_active = $this->is_bonus_for_woo_active();

		if ( ! $is_bfw_active ) {
			echo '<p class="description">'
				. esc_html__( 'Bonus For Woo is not active. Activate the plugin to enable status copying.', 'kd-bonus' )
				. '</p>';
			return;
		}

		$action_url = admin_url( 'admin-post.php' );
		$return_url = is_network_admin()
		...
		<form method="post" action="<?php echo esc_url( $action_url ); ?>" style="display:inline;">
			<?php wp_nonce_field( 'kd_bonus_copy_status_bfw_' . $user_id, 'kd_bonus_copy_status_bfw_nonce' ); ?>
			<input type="hidden" name="action" value="kd_bonus_copy_status_bfw" />
			<input type="hidden" name="user_id" value="<?php echo esc_attr( (string) $user_id ); ?>" />
			<input type="hidden" name="kd_bonus_return_url" value="<?php echo esc_attr( $return_url ); ?>" />
			<button type="submit" class="button button-secondary">
				<?php esc_html_e( 'Copy Status from Bonus For Woo', 'kd-bonus' ); ?>
			</button>
		</form>
		<p class="description"><?php esc_html_e( 'Reads the client status from Bonus For Woo and saves it as a KD Bonus membership override.', 'kd-bonus' ); ?></p>
		<?php
	}

	/**
	 * Handle the admin-post action for copying Bonus For Woo status.
	 */
	public function handle_copy_status_from_bfw() {
		if ( empty( $_POST['user_id'] ) || empty( $_POST['kd_bonus_copy_status_bfw_nonce'] ) ) {
			wp_die( esc_html__( 'Missing required fields.', 'kd-bonus' ), 403 );
		}

		$user_id = absint( $_POST['user_id'] );
		if ( $user_id <= 0 ) {
			wp_die( esc_html__( 'Invalid user.', 'kd-bonus' ), 403 );
		}

		if ( ! wp_verify_nonce(
			sanitize_text_field( wp_unslash( $_POST['kd_bonus_copy_status_bfw_nonce'] ) ),
			'kd_bonus_copy_status_bfw_' . $user_id
		) ) {
			wp_die( esc_html__( 'Security check failed.', 'kd-bonus' ), 403 );
		}

		if ( ! $this->can_manage_user_bonus( $user_id ) ) {
			wp_die( esc_html__( 'You do not have permission to manage this user.', 'kd-bonus' ), 403 );
		}

		$return_url = wp_validate_redirect(
			sanitize_url( wp_unslash( $_POST['kd_bonus_return_url'] ?? '' ) ),
			is_network_admin()
				? network_admin_url( 'user-edit.php?user_id=' . $user_id )
				: admin_url( 'user-edit.php?user_id=' . $user_id )
		);

		$result = $this->copy_status_from_bonus_for_woo( $user_id );

		$return_url = add_query_arg(
			array(
				'kd_bonus_bfw_copy' => rawurlencode( $result['status'] ),
				'kd_bonus_bfw_msg'  => rawurlencode( $result['message'] ),
			),
			$return_url
		);
	...
	/**
	 * Read the Bonus For Woo client status for a user, map it to a KD Bonus membership
	 * status, and save it as an override.
	 *
	 * @param int $user_id Target user ID.
	 * @return array{status: string, message: string} Result with 'success'/'error' status and a descriptive message.
	 */
	public function copy_status_from_bonus_for_woo( $user_id ) {
		$user_id = absint( $user_id );

		if ( ! $this->is_bonus_for_woo_active() ) {
			return array(
				'status'  => 'error',
				'message' => __( 'Bonus For Woo is not active. Please activate the plugin and try again.', 'kd-bonus' ),
			);
		}

		$bfw_status = $this->get_bonus_for_woo_client_status( $user_id );

		if ( '' === $bfw_status ) {
			return array(
				'status'  => 'error',
				'message' => __( 'No Bonus For Woo client status found for this customer. Nothing was changed.', 'kd-bonus' ),
			);
		}

		$matched = $this->map_bfw_status_to_kd_bonus( $bfw_status );

		if ( null === $matched ) {
			return array(
				'status'  => 'error',
				/* translators: %s: the Bonus For Woo status value that could not be mapped. */
				'message' => sprintf(
					__( 'Bonus For Woo status "%s" could not be mapped to any KD Bonus membership status. Nothing was changed.', 'kd-bonus' ),
					$bfw_status
				),
			);
		}
		...
		update_user_meta( $user_id, self::STATUS_OVERRIDE_META, $matched );
		$effective_after = $this->get_user_status( $user_id );
		update_user_meta( $user_id, self::STATUS_META, sanitize_text_field( $effective_after['name'] ?? '' ) );
		...
		if ( ( $effective_before['name'] ?? '' ) !== ( $effective_after['name'] ?? '' ) ) {
			$this->record_status_change_event(
				...
				$effective_after['name'] ?? '',
				sprintf(
					/* translators: 1: previous status, 2: new status, 3: source Bonus For Woo status. */
					__( 'Administrator copied membership status from Bonus For Woo: %1$s → %2$s (source: "%3$s").', 'kd-bonus' ),
					$effective_before['name'] ?: __( 'No status', 'kd-bonus' ),
					$effective_after['name'] ?: __( 'No status', 'kd-bonus' ),
					$bfw_status
				),
		...
		return array(
			'status'  => 'success',
			/* translators: 1: Bonus For Woo source status, 2: KD Bonus status applied. */
			'message' => sprintf(
				__( 'Status copied from Bonus For Woo. Source: "%1$s" → KD Bonus override set to "%2$s".', 'kd-bonus' ),
				$bfw_status,
				$matched
			),
		);
	...
	/**
	 * Determine whether Bonus For Woo (by Computy) is active on the current site.
	 *
	 * @return bool
	 */
	protected function is_bonus_for_woo_active() {
		if ( ! function_exists( 'is_plugin_active' ) ) {
			require_once ABSPATH . 'wp-admin/includes/plugin.php';
		}

		// Support the filter so site owners can override auto-detection.
		$override = apply_filters( 'kd_bonus_bfw_active_override', null );
		if ( null !== $override ) {
			return (bool) $override;
		}

		// Check for any plugin whose slug begins with "bonus-for-woo".
		if ( function_exists( 'is_plugin_active' ) ) {
			foreach ( (array) get_option( 'active_plugins', array() ) as $plugin_file ) {
				if ( false !== strpos( (string) $plugin_file, 'bonus-for-woo' ) ) {
					return true;
				}
			}
		...
		// Fallback: check whether the primary Bonus For Woo class exists.
		return class_exists( 'BonusForWoo' ) || class_exists( 'Bonus_For_Woo' ) || class_exists( 'ComputyBonusForWoo' );
	}

	/**
	 * Read the raw client status stored by Bonus For Woo for a given user.
	 *
	 * Bonus For Woo (Computy) stores the client type in user meta.  The exact
	 * meta key can vary by version; we probe the known keys in priority order
	 * and return the first non-empty value found.
	 *
	 * Site owners may override the meta key via the `kd_bonus_bfw_status_meta_key`
	 * filter if their installation uses a different key.
	 *
	 * @param int $user_id User ID.
	 * @return string Raw status string, or empty string when not found.
	 */
	protected function get_bonus_for_woo_client_status( $user_id ) {
		$user_id = absint( $user_id );

		/**
		 * Override the Bonus For Woo user-meta key used to look up the client status.
		 *
		 * Return a non-empty string to use a single custom key, or an array of keys
		 * to probe in order (first non-empty value wins).
		 *
		 * @param string|string[]|null $key     Custom key(s), or null to use defaults.
		 * @param int                  $user_id User being queried.
		 */
		$custom_keys = apply_filters( 'kd_bonus_bfw_status_meta_key', null, $user_id );

		if ( null !== $custom_keys ) {
			$probe_keys = (array) $custom_keys;
		} else {
			// Known meta keys used by Bonus For Woo across versions.
			$probe_keys = array(
				'bonuswoo_client_type',
				'_bonuswoo_client_type',
				'bonus_for_woo_client_type',
				'_bonus_for_woo_client_type',
				'computy_bonus_client_type',
				'_computy_bonus_client_type',
				'bfw_client_type',
				'bfw_client_status',
			);
		}
		...
		foreach ( $probe_keys as $key ) {
			$value = trim( (string) get_user_meta( $user_id, sanitize_key( $key ), true ) );
			if ( '' !== $value ) {
				return $value;
			}
		}
	...
	/**
	 * Map a raw Bonus For Woo status string to a KD Bonus membership status name.
	 *
	 * Resolution order:
	 *  1. Case-insensitive exact match against KD Bonus status names.
	 *  2. Site-owner-provided mapping via the `kd_bonus_bfw_status_map` filter.
	 *  3. Returns null when no match is found.
	 *
	 * @param string $bfw_status Raw status from Bonus For Woo.
	 * @return string|null Matched KD Bonus status name, or null when unmatched.
	 */
	protected function map_bfw_status_to_kd_bonus( $bfw_status ) {
		$bfw_status = trim( $bfw_status );
		if ( '' === $bfw_status ) {
			return null;
		}

		$kd_statuses = $this->get_membership_statuses();

		// 1. Case-insensitive exact match.
		foreach ( $kd_statuses as $kd_status ) {
			if ( 0 === strcasecmp( $bfw_status, (string) ( $kd_status['name'] ?? '' ) ) ) {
				return (string) $kd_status['name'];
			}
		}

		/**
		 * Provide a custom Bonus For Woo → KD Bonus status mapping.
		 *
		 * Return an associative array where each key is a Bonus For Woo status
		 * string (case-insensitive) and each value is the target KD Bonus status name.
		 *
		 * Example:
		 *   [ 'gold' => 'Gold Member', 'silver' => 'Silver Member' ]
		 *
		 * @param array<string,string> $map     Empty by default.
		 * @param string               $bfw_status The raw Bonus For Woo status being mapped.
		 */
		$custom_map = (array) apply_filters( 'kd_bonus_bfw_status_map', array(), $bfw_status );

		foreach ( $custom_map as $source => $target ) {
			if ( 0 === strcasecmp( $bfw_status, (string) $source ) ) {
				$matched_status = $this->get_status_by_name( (string) $target );
				if ( ! empty( $matched_status ) ) {
					return (string) $matched_status['name'];
				}
			}
		}
	...
	/**
	 * Display an admin notice after a Copy Status from Bonus For Woo action.
	 */
	public function render_copy_status_from_bfw_notice() {
		if ( empty( $_GET['kd_bonus_bfw_copy'] ) || empty( $_GET['kd_bonus_bfw_msg'] ) ) {
			return;
		}

		$copy_status = sanitize_key( wp_unslash( $_GET['kd_bonus_bfw_copy'] ) );
		$message     = sanitize_text_field( urldecode( wp_unslash( $_GET['kd_bonus_bfw_msg'] ) ) );

		if ( '' === $message ) {
			return;
		}

		$type = ( 'success' === $copy_status ) ? 'updated' : 'error';
		printf(
