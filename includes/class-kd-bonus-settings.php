...
class KD_Bonus_Settings {
	...
	private function render_membership_status_rows( $settings ) {
		...
		<tr>
			...
			<td>
				<p class="description"><?php esc_html_e( 'Add, edit, or delete as many statuses as needed. Rows are saved and displayed in ascending priority order (1 = first, 2 = second, and so on), then renumbered starting from 1. Membership resolution uses required spend first; when thresholds match, the lower priority number takes precedence. Reward percentage is applied to eligible product subtotal only.', 'kd-bonus' ); ?></p>
